...
func TestFunctionTool_RunParsesRawJSONArgs(t *testing.T) {
	sumTool, err := functiontool.New(functiontool.Config{
		Name:        "sum",
		Description: "sums two integers",
	}, sumFunc)
	...
	funcTool, ok := sumTool.(toolinternal.FunctionTool)
	if !ok {
		t.Fatal("sumTool does not implement toolinternal.FunctionTool")
	}
	...
	if got, want := result["sum"], any(float64(5)); got != want {
		t.Fatalf("Run() sum = %#v, want %#v", got, want)
	}
}
