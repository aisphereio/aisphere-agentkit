...
func New(cfg Config) (tool.Toolset, error) {
	return &set{
		...
		toolNamePrefix:              cfg.ToolNamePrefix,
	}, nil
}
...
type Config struct {
	...
	// ToolNamePrefix exposes MCP tools to the model with a stable namespace.
	// Example: prefix "novel_assets" turns remote tool "list_split_books"
	// into ADK tool "novel_assets__list_split_books" while still calling
	// the remote MCP tool as "list_split_books". This prevents collisions
	// when one agent binds multiple MCP servers.
...
func (s *set) Tools(ctx agent.ReadonlyContext) ([]tool.Tool, error) {
	...
	for _, mcpTool := range mcpTools {
		t, err := convertTool(mcpTool, s.mcpClient, s.requireConfirmation, s.requireConfirmationProvider, s.toolNamePrefix)
		if err != nil {
