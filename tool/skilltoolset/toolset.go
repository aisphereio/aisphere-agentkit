...
` +
		"1. If a skill seems relevant to the current user query, you MUST use the `load_skill` tool with `name=\"<SKILL_NAME>\"` to read its full instructions before proceeding. The legacy `skill_name` argument is accepted for compatibility, but `name` is preferred.\n" +
