...
func TestLoadArtifactsTool_ProcessRequest_TruncatesLargeArtifact(t *testing.T) {
	loadArtifactsTool := loadartifactstool.New()

	tc := createToolContext(t)
	largeText := strings.Repeat("x", 70000)
	if _, err := tc.Artifacts().Save(t.Context(), "large.txt", genai.NewPartFromText(largeText)); err != nil {
	...
	functionResponse := &genai.FunctionResponse{
		Name: "load_artifacts",
		Response: map[string]any{
			"artifact_names": []string{"large.txt"},
		},
	}
	llmRequest := &model.LLMRequest{
		Contents: []*genai.Content{
	...
	text := appendedContent.Parts[1].Text
	if len([]rune(text)) > 65000 {
		t.Fatalf("loaded artifact text length = %d, want capped below 65000", len([]rune(text)))
	}
	if !strings.Contains(text, "truncated") {
		t.Fatalf("loaded artifact text should include truncation warning, got prefix %q", text[:min(80, len(text))])
	}
}
