...
const defaultMaxLoadedArtifactChars = 64000
	...
	return &genai.FunctionDeclaration{
		...
		Parameters: &genai.Schema{
			...
			Properties: map[string]*genai.Schema{
				...
				"max_chars": {
					Type:        "INTEGER",
					Description: "Optional maximum text characters to append per artifact. Defaults to 64000.",
				},
			},
		},
	}
...
func (t *artifactsTool) Run(ctx tool.Context, args any) (map[string]any, error) {
	...
	if maxChars := intFromAny(m["max_chars"]); maxChars > 0 {
...
func (t *artifactsTool) appendInitialInstructions(ctx tool.Context, req *model.LLMRequest) error {
	...
	artifactNamesJSON, err := json.Marshal(fileNames)
	if err != nil {
...
func (t *artifactsTool) processLoadArtifactsFunctionCall(ctx tool.Context, req *model.LLMRequest) error {
	...
	maxChars := defaultMaxLoadedArtifactChars
	if v := intFromAny(functionResponse.Response["max_chars"]); v > 0 {
		maxChars = v
	}
	...
	for i, artifactName := range artifactNames {
		i, artifactName := i, artifactName
		group.Go(func() error {
			content, err := t.loadIndividualArtifact(childCtx, artifactsService, artifactName, maxChars)
			if err != nil {
				// A stale registry/run-state pointer should not crash the whole agent run.
				// Return a visible warning as content so the model can pick a newer artifact
				// or restart the domain run, while the platform keeps running.
				results[i] = &genai.Content{
					Parts: []*genai.Part{
						genai.NewPartFromText(fmt.Sprintf("Artifact %s could not be loaded: %v", artifactName, err)),
					},
					Role: genai.RoleUser,
				}
				return nil
...
func (t *artifactsTool) loadIndividualArtifact(ctx context.Context, artifactsService agent.Artifacts, artifactName string, maxChars int) (*genai.Content, error) {
	resp, err := artifactsService.Load(ctx, artifactName)
	if err != nil {
		return nil, fmt.Errorf("failed to load artifact %s: %w", artifactName, err)
	}
	part := boundedArtifactPart(artifactName, resp.Part, maxChars)
...
func boundedArtifactPart(artifactName string, part *genai.Part, maxChars int) *genai.Part {
	if part == nil {
		return genai.NewPartFromText(fmt.Sprintf("Artifact %s is empty.", artifactName))
	}
	if maxChars <= 0 {
		maxChars = defaultMaxLoadedArtifactChars
	}
	if part.Text != "" {
		return genai.NewPartFromText(truncateForModel(part.Text, maxChars, artifactName))
	}
	if part.InlineData == nil {
		return part
	}
	mimeType := strings.ToLower(strings.TrimSpace(part.InlineData.MIMEType))
	if strings.Contains(mimeType, "text") || strings.Contains(mimeType, "json") || strings.Contains(mimeType, "xml") || strings.Contains(mimeType, "markdown") {
		return genai.NewPartFromText(truncateForModel(string(part.InlineData.Data), maxChars, artifactName))
	}
	if int64(len(part.InlineData.Data)) > int64(maxChars*4) {
		return genai.NewPartFromText(fmt.Sprintf("Artifact %s is binary or non-text and was not appended because it is too large (%d bytes). Use a preview/conversion tool instead.", artifactName, len(part.InlineData.Data)))
	}
	return part
}

func truncateForModel(text string, maxChars int, artifactName string) string {
	if maxChars <= 0 || utf8.RuneCountInString(text) <= maxChars {
		return text
	}
	runes := []rune(text)
	return string(runes[:maxChars]) + fmt.Sprintf("\n\n[truncated: artifact %s exceeded max_chars=%d; load a smaller artifact or pass a lower max_chars for previews]", artifactName, maxChars)
}
