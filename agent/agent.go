...
func (a *agent) Run(ctx InvocationContext) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		startedAt := time.Now()
		status := "completed"
		runtimetrace.Record(ctx, runtimetrace.EventAgentEnter, map[string]any{
			"agent_name":      a.Name(),
			"agent_class":     fmt.Sprintf("%T", a),
			"agent_type":      a.State.AgentType,
			"description":     a.Description(),
			"sub_agent_count": len(a.SubAgents()),
		})
		defer func() {
			runtimetrace.Record(ctx, runtimetrace.EventAgentExit, map[string]any{
				"agent_name":  a.Name(),
				"agent_class": fmt.Sprintf("%T", a),
				"agent_type":  a.State.AgentType,
				"status":      status,
		...
		if event != nil || err != nil {
			if err != nil {
				status = "failed"
				runtimetrace.Record(ctx, runtimetrace.EventAgentError, map[string]any{
					"agent_name": a.Name(),
					"stage":      "before_agent_callback",
					"error":      err.Error(),
				})
			}
			if !yield(event, err) {
