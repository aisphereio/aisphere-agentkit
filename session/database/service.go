...
// ListAll retrieves sessions across apps/users for admin use. Returned sessions do
// not include events, matching the existing List behavior.
func (s *databaseService) ListAll(ctx context.Context, req *session.ListAllRequest) (*session.ListResponse, error) {
	if req == nil {
		req = &session.ListAllRequest{}
	}
	limit := req.Limit
	if limit <= 0 || limit > 200 {
		limit = 50
	}
	query := s.db.WithContext(ctx).Model(&storageSession{})
	if req.AppName != "" {
		query = query.Where("app_name = ?", req.AppName)
	}
	if req.UserID != "" {
	...
	var foundSessions []storageSession
	if err := query.Order("update_time DESC").Limit(limit).Find(&foundSessions).Error; err != nil {
		return nil, fmt.Errorf("database error while listing sessions: %w", err)
	}

	responseSessions := make([]session.Session, 0, len(foundSessions))
	for i := range foundSessions {
		storageSess := &foundSessions[i]
		responseSession, err := s.sessionFromStorage(ctx, storageSess, nil)
		if err != nil {
			return nil, err
		}
...
// GetByID locates a session by session id for admin use, then returns it with
// events and merged app/user/session state.
func (s *databaseService) GetByID(ctx context.Context, req *session.GetByIDRequest) (*session.GetResponse, error) {
	if req == nil || req.SessionID == "" {
		return nil, fmt.Errorf("session_id is required")
	}
	var foundSession storageSession
	if err := s.db.WithContext(ctx).Where("id = ?", req.SessionID).Order("update_time DESC").First(&foundSession).Error; err != nil {
		return nil, fmt.Errorf("database error while fetching session: %w", err)
	}
	return s.Get(ctx, &session.GetRequest{AppName: foundSession.AppName, UserID: foundSession.UserID, SessionID: foundSession.ID})
}

func (s *databaseService) sessionFromStorage(ctx context.Context, storageSess *storageSession, events []*session.Event) (*localSession, error) {
	responseSession, err := createSessionFromStorageSession(storageSess)
	if err != nil {
		return nil, fmt.Errorf("failed to map storage object for session %s: %w", storageSess.ID, err)
	}
	storageApp, err := fetchStorageAppState(s.db.WithContext(ctx), storageSess.AppName)
	if err != nil {
		return nil, fmt.Errorf("error while fetching app state for session %s: %w", storageSess.ID, err)
	}
	storageUser, err := fetchStorageUserState(s.db.WithContext(ctx), storageSess.AppName, storageSess.UserID)
	if err != nil {
		return nil, fmt.Errorf("error while fetching user state for session %s: %w", storageSess.ID, err)
	}
	responseSession.state = mergeStates(storageApp.State, storageUser.State, responseSession.state)
	responseSession.events = events
...
func fetchStorageAppState(tx *gorm.DB, appName string) (*storageAppState, error) {
	...
	result := tx.Where("app_name = ?", appName).Limit(1).Find(&storageApp)
	if result.Error != nil {
...
func fetchStorageUserState(tx *gorm.DB, appName, userID string) (*storageUserState, error) {
	...
	result := tx.Where("app_name = ? AND user_id = ?", appName, userID).Limit(1).Find(&storageUser)
	if result.Error != nil {
