...
func TestCreateSessionBindsProjectIDFromQuery(t *testing.T) {
	id := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
		SessionID: "testSession",
	...
	apiController := controllers.NewSessionsAPIController(&sessionService)
	req, err := http.NewRequest(http.MethodPost, "/apps/testApp/users/testUser/sessions/testSession?project_id=project_a", bytes.NewBufferString(`{"state":{"foo":"bar"}}`))
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	...
	if got, want := stored.SessionState["project_id"], "project_a"; got != want {
		t.Fatalf("project_id state = %v, want %q", got, want)
	}
	if got, want := stored.SessionState["projectId"], "project_a"; got != want {
		t.Fatalf("projectId state = %v, want %q", got, want)
	}
	if got, want := stored.SessionState["foo"], "bar"; got != want {
		t.Fatalf("existing state foo = %v, want %q", got, want)
	}
}

func TestCreateSessionBindsProjectIDFromCurrentSession(t *testing.T) {
	currentID := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
		SessionID: "currentSession",
	}
	newID := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
	...
	sessionService := fakes.FakeSessionService{Sessions: map[fakes.SessionKey]fakes.TestSession{
		currentID: {
			Id:            currentID,
			SessionState:  fakes.TestState{"project_id": "project_a"},
			SessionEvents: fakes.TestEvents{},
			UpdatedAt:     time.Now(),
		},
	}}
	apiController := controllers.NewSessionsAPIController(&sessionService)
	req, err := http.NewRequest(http.MethodPost, "/apps/testApp/users/testUser/sessions/newSession?current_session_id=currentSession", bytes.NewBufferString(`{"state":{"foo":"bar"}}`))
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req = mux.SetURLVars(req, sessionVars(newID))
	rr := httptest.NewRecorder()
	...
	stored := sessionService.Sessions[newID]
	if got, want := stored.SessionState["project_id"], "project_a"; got != want {
		t.Fatalf("project_id state = %v, want %q", got, want)
	}
}
...
func TestListSessionsFiltersByProjectID(t *testing.T) {
	projectA := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
		SessionID: "projectA",
	}
	projectB := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
		SessionID: "projectB",
	}
	unbound := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
		SessionID: "unbound",
	}
	sessionService := fakes.FakeSessionService{Sessions: map[fakes.SessionKey]fakes.TestSession{
		...
		projectB: {
			Id:            projectB,
			SessionState:  fakes.TestState{"project_id": "project_b"},
			SessionEvents: fakes.TestEvents{},
			UpdatedAt:     time.Now(),
		},
		unbound: {
	...
	apiController := controllers.NewSessionsAPIController(&sessionService)
	req, err := http.NewRequest(http.MethodGet, "/apps/testApp/users/testUser/sessions?project_id=project_a", nil)
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req = mux.SetURLVars(req, map[string]string{
	...
	want := []models.Session{{
		...
		UpdatedAt: time.Now().Unix(),
		Events:    []models.Event{},
		State:     map[string]any{"project_id": "project_a"},
	}}
	if diff := cmp.Diff(want, got, EquateApproxInt(int64(time.Second))); diff != "" {
		t.Errorf("ListSessions() mismatch (-want +got):\n%s", diff)
	}
}

func TestListSessionsFiltersByCurrentSessionProjectID(t *testing.T) {
	currentID := fakes.SessionKey{
		AppName:   "testApp",
		UserID:    "testUser",
		SessionID: "currentSession",
	...
	apiController := controllers.NewSessionsAPIController(&sessionService)
	req, err := http.NewRequest(http.MethodGet, "/apps/testApp/users/testUser/sessions?current_session_id=currentSession", nil)
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req = mux.SetURLVars(req, map[string]string{
	...
	wantIDs := map[string]bool{"currentSession": true, "projectA": true}
	if len(got) != len(wantIDs) {
		t.Fatalf("session count = %d, want %d: %+v", len(got), len(wantIDs), got)
	}
	for _, sess := range got {
		if !wantIDs[sess.ID] {
			t.Fatalf("unexpected session %q in filtered list: %+v", sess.ID, got)
		}
	}
}
