...
func (w *webUILauncher) SetupSubrouters(router *mux.Router, config *launcher.Config) error {
	w.AddSubrouter(router, w.config.pathPrefix, w.config.backendAddress, config.RuntimeConfig)
	return nil
}
...
func (w *webUILauncher) AddSubrouter(router *mux.Router, pathPrefix, backendAddress string, runtimeCfg *runtimeconfig.Config) {
	...
	cfg := runtimeCfg
	if cfg == nil {
		cfg = runtimeconfig.FromContext(nil)
	}
	runtimeConfigResponse := struct {
		BackendUrl      string                                `json:"backendUrl"`
		Models          []runtimeconfig.FrontendModel         `json:"models"`
		Tools           []runtimeconfig.FrontendTool          `json:"tools"`
		MCPServers      []runtimeconfig.FrontendMCPServer     `json:"mcpServers"`
		BuilderDefaults runtimeconfig.FrontendBuilderDefaults `json:"builderDefaults"`
	}{
		BackendUrl:      backendAddress,
		Models:          cfg.FrontendModels(),
		Tools:           cfg.FrontendTools(),
		MCPServers:      cfg.FrontendMCPServers(),
		BuilderDefaults: cfg.FrontendBuilderDefaults(),
	}
