...
// WithInvocationID pins the platform invocation id for one run. Runtime
// controllers use this so SSE metadata, persisted session events, and trace
// files all reference the same id.
...
func mergeRunStateDeltaIntoSession(sess session.Session, delta map[string]any) {
	if sess == nil || sess.State() == nil || len(delta) == 0 {
		return
	}
	for key, value := range delta {
		_ = sess.State().Set(key, value)
	}
}
...
func (r *Runner) Run(ctx context.Context, userID, sessionID string, msg *genai.Content, cfg agent.RunConfig, opts ...RunOption) iter.Seq2[*session.Event, error] {
	...
	return func(yield func(*session.Event, error) bool) {
		...
		if options.invocationID == "" {
			options.invocationID = "e-" + uuid.NewString()
		}
		if rec := runtimetrace.FromContext(ctx); rec != nil && rec.Enabled() {
			rec.Record(ctx, runtimetrace.Event{
				Type:         runtimetrace.EventInvocationStarted,
				...
				Data: map[string]any{
					...
					"root_agent":    r.rootAgent.Name(),
					"has_message":   msg != nil,
					"streaming":     cfg.StreamingMode,
					"invocation_id": options.invocationID,
				},
			})
		}
		defer func() {
			eventType := runtimetrace.EventInvocationCompleted
			if status == "failed" {
				eventType = runtimetrace.EventInvocationFailed
			}
			if rec := runtimetrace.FromContext(ctx); rec != nil && rec.Enabled() {
				rec.Record(ctx, runtimetrace.Event{
					Type:         eventType,
		...
		mergeRunStateDeltaIntoSession(storedSession, options.stateDelta)
		...
		runtimetrace.Record(ctx, runtimetrace.EventAgentSelected, map[string]any{
			"app_name":       r.appName,
			"user_id":        userID,
			"session_id":     storedSession.ID(),
			"selected_agent": agentToRun.Name(),
			"root_agent":     r.rootAgent.Name(),
		})
