...
func (ia *internalArtifacts) Delete(ctx context.Context, name string) error {
	type deleteCapable interface {
		Delete(context.Context, string) error
	}
	artifacts, ok := ia.Artifacts.(deleteCapable)
	if !ok {
		return fmt.Errorf("artifact service does not support delete through this context")
	}
	return artifacts.Delete(ctx, name)
}
