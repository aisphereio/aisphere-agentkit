...
func (f *Flow) preprocess(ctx agent.InvocationContext, req *model.LLMRequest) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		tracePreprocess(ctx, "llm.preprocess.before", req)
		defer tracePreprocess(ctx, "llm.preprocess.after", req)
...
func generateContent(ctx agent.InvocationContext, m model.LLM, req *model.LLMRequest, useStream bool) iter.Seq2[*responseWithEventID, error] {
	return func(yield func(*responseWithEventID, error) bool) {
		// Record the final ADK LLM request before the context-budget guard.
		// This is intentionally before validateLLMRequestContextBudget so oversized
		// requests can still be inspected from /api/runtime/traces.
		traceLLMRequest(ctx, req, useStream)
		if err := validateLLMRequestContextBudget(req, defaultLLMContextWindow); err != nil {
			traceLLMRequestBudget(ctx, req, defaultLLMContextWindow, err)
			yield(nil, err)
			return
		}
		traceLLMRequestBudget(ctx, req, defaultLLMContextWindow, nil)
...
func (f *Flow) handleFunctionCalls(ctx agent.InvocationContext, toolsDict map[string]tool.Tool, resp *model.LLMResponse, toolConfirmations map[string]*toolconfirmation.ToolConfirmation, liveSess agent.LiveSession) (mergedEvent *session.Event, err error) {
	...
	for i, fnCall := range fnCalls {
		...
		go func(i int, fnCall *genai.FunctionCall) {
			...
			var toolErrForTrace error
			if result != nil {
				if errValue, ok := result["error"]; ok && errValue != nil {
					if errString, ok := errValue.(string); ok {
						toolErrForTrace = errors.New(errString)
					}
				}
			}
			traceToolResult(ctx, fnCall, result, toolErrForTrace)
