...
func testArtifactService_UserScoped(ctx context.Context, t *testing.T, srv artifact.Service, testSuffix string) {
	...
	t.Run(fmt.Sprintf("UserScopedCrossApp_%s", testSuffix), func(t *testing.T) {
		if _, err := srv.Save(ctx, &artifact.SaveRequest{
			AppName:   "source_app",
			UserID:    userID,
			SessionID: "source_session",
			FileName:  "user:shared-project-artifact",
			Part:      genai.NewPartFromText("shared across apps"),
		}); err != nil {
			t.Fatalf("Save(user scoped cross-app) failed: %v", err)
		}
		resp, err := srv.Load(ctx, &artifact.LoadRequest{
			AppName:   "target_app",
			UserID:    userID,
		...
		if err != nil {
			t.Fatalf("Load(user scoped from another app) failed: %v", err)
		}
		if diff := cmp.Diff(resp.Part, genai.NewPartFromText("shared across apps")); diff != "" {
			t.Fatalf("Load(user scoped from another app) mismatch (-got +want):\n%s", diff)
		}
		listResp, err := srv.List(ctx, &artifact.ListRequest{
			AppName:   "target_app",
			UserID:    userID,
			SessionID: "target_session",
		})
		if err != nil {
			t.Fatalf("List(target app) failed: %v", err)
		}
		if !slices.Contains(listResp.FileNames, "user:shared-project-artifact") {
			t.Fatalf("List(target app) = %v, want user:shared-project-artifact", listResp.FileNames)
		}
	})
	...
	t.Run(fmt.Sprintf("ListAfterDelete_%s", testSuffix), func(t *testing.T) {
		...
		got := resp.FileNames
		slices.Sort(got)
		want := []string{"file2", "user:file3", "user:shared-project-artifact"} // testData plus cross-app user artifact.
		if diff := cmp.Diff(got, want); diff != "" {
			t.Errorf("List() = %v, want %v", got, want)
		}
	})
